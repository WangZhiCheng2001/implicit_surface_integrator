...
class flat_multiset_base_comp : private Compare
{
protected:
    template <typename Key>
    constexpr flat_multiset_base_comp(Key&& t) noexcept(std::is_nothrow_constructible_v<Compare, Key>)
...
class flat_multiset_base_comp<Compare, false>
{
protected:
    template <typename Key>
    constexpr flat_multiset_base_comp(Key&& t) noexcept(std::is_nothrow_constructible_v<Compare, Key>)
...
template <typename Impl,
          ...
          typename Key,
          typename Compare,
          typename RKey = typename Vector<Key>::value_type>
class flat_multiset_base : private flat_multiset_base_comp<Compare>
{
    using mybase = flat_multiset_base_comp<Compare>;
...
public:
    ...
    flat_multiset_base() : mybase(Compare()) {}

    explicit flat_multiset_base(const container_type& sorted_storage, const Compare& comp = Compare())
        : mybase(comp), storage(sorted_storage)
    {
        assert(std::is_sorted(begin(), end(), this->GetCompare()));
    }

    explicit flat_multiset_base(container_type&& sorted_storage, const Compare& comp = Compare())
        : mybase(comp), storage(std::move(sorted_storage))
    {
        assert(std::is_sorted(begin(), end(), this->GetCompare()));
    }

    explicit flat_multiset_base(const Compare& comp) : mybase(comp) {}

    template <typename Iter>
        requires std::input_iterator<Iter>
    flat_multiset_base(Iter first, Iter last, const Compare& comp = Compare()) : mybase(comp)
    {
        insert(first, last);
    }

    flat_multiset_base(const flat_multiset_base&) = default;

    flat_multiset_base(flat_multiset_base&&) noexcept = default;

    flat_multiset_base(std::initializer_list<value_type> ilist, const Compare& comp = Compare())
        : flat_multiset_base(ilist.begin(), ilist.end(), comp)
    {
    }

    Impl& operator=(const flat_multiset_base& rhs)
    {
        storage = rhs.storage;
        return static_cast<Impl&>(*this);
    }

    Impl& operator=(flat_multiset_base&& rhs) noexcept
    {
    ...
    const_iterator find(const key_type& key) const { return const_cast<flat_multiset_base*>(this)->find(key); }
    ...
    iterator lower_bound(const key_type& key) { return std::lower_bound(begin(), end(), key, this->GetCompare()); }

    const_iterator lower_bound(const key_type& key) const { return std::lower_bound(begin(), end(), key, this->GetCompare()); }

    iterator upper_bound(const key_type& key) { return std::upper_bound(begin(), end(), key, this->GetCompare()); }

    const_iterator upper_bound(const key_type& key) const { return std::upper_bound(begin(), end(), key, this->GetCompare()); }
    ...
    {
        return const_cast<flat_multiset_base*>(this)->find(key);
    }
...
private:
    ...
    template <typename V>
        requires std::is_same<value_type, std::remove_cvref_t<V>>::value
    ...
    iterator emplace_hint_impl(const_iterator hint) { return emplace_hint_impl(hint, value_type()); }

    template <typename Arg, typename... Args>
        requires std::bool_constant<sizeof...(Args) != 0>::value
...
bool operator==(const flat_multiset_base<Impl, IsMulti, Vector, Key, Compare>& lhs,
                const flat_multiset_base<Impl, IsMulti, Vector, Key, Compare>& rhs)
{
